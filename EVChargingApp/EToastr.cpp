...
EToastr::EToastr(QMainWindow *parent, bool showIcon)
	: QFrame(parent)
	, ui(new Ui::Toastr)
{
	ui->setupUi(this);
	...
	setParent(_parent);
    if (parent) {
        QObject::connect(_parent, SIGNAL(resizeEvent(QResizeEvent*)), this, SLOT(SlotParentResized(QResizeEvent*)));
        QObject::connect(_parent, SIGNAL(moveEvent(QMoveEvent*)), this, SLOT(SlotParentMoved(QMoveEvent*)));
	}
