...
public:
    ...
    EToastr(QMainWindow *parent = nullptr, bool showIcon = true); // if parent is null, Desktop will be the parent.
    ~EToastr();
...
private:
    bool m_closing = false; // When toastr hiding, if user resize or move parent widget it will cause a bug, for fix that. (Toastr tam kapanacakken parent boyutu yada konumu de?i?ince ?nceki animasyon ile ?ak???yor) bu bool sayesinde 2. animasyon ignore edilecek e?er kapan?yorsa
    Ui::Toastr* ui;
